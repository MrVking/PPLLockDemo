//
//  PPLDeviceTableViewController.m
//  PPLLockDemo
//
...
#import "PPLDeviceTableViewController.h"
#import "PPLOnFoundDeviceModel.h"
#import "PPLDeviceCell.h"
#import "PPLDemoViewController.h"
#import "PPLNetworkHelper.h"
...
@interface PPLDeviceTableViewController ()<PPLLockDelegate>
{
    PPLOnFoundDeviceModel * _selectModel;
...
@implementation PPLDeviceTableViewController
...
- (void)setupTableView
{
    ...
    [self.tableView registerNib:[UINib nibWithNibName:NSStringFromClass([PPLDeviceCell class]) bundle:nil] forCellReuseIdentifier:lockDeviceCellID];
...
{
    PPLDeviceCell * cell = [tableView dequeueReusableCellWithIdentifier:lockDeviceCellID];
    
    PPLOnFoundDeviceModel *blueModel = self.blueListArr[indexPath.row];
...
- (void)didFindPeripheral:(CBPeripheral *)peripheral lockName:(NSString *)lockName mac:(NSString *)mac hasBind:(BOOL)hasBind
{
    ...
    for (int i = 0;  i < count; i++) {
        PPLOnFoundDeviceModel *model = [[PPLOnFoundDeviceModel alloc]init];
...
- (void)getAccessToken
{
    ...
    [PPLNetworkHelper requestExperUserKeyWithAppKey:AppKey appSecrect:Appsecret Handler:^(int code, NSString *data, NSError *error) {
